...
AATIF originated from 73 governance conjectures derived through a year of sustained observation across four LLM platforms (ChatGPT, Claude, Gemini, DeepSeek), documented in 78 field notes available as supplementary material. This paper operationalizes one of those conjectures---Conjecture~\#063, that governance operates through probabilistic constraint density rather than rule retrieval---into a computational engine with an explicit mathematical formulation, a comprehensive test suite of 437 deterministic tests, and benchmark evaluation against HarmBench and MultiJail.
...
\paragraph{Harm gate.} $[1 - \sigma(\alpha(H - \theta))] \in (0,1)$ is a soft gate controlled by semantic harm proximity $H$. The steepness $\alpha = 10$ controls transition sharpness; the threshold $\theta = 0.40$ sets where the gate begins to close. When $H \ll \theta$ the gate is open ($\approx 1$); when $H \gg \theta$ it closes ($\approx 0$), driving $S$ toward zero.
...
\paragraph{Sigmoid symmetry.} The harm gate satisfies $[1 - \sigma(\alpha(H - \theta))] = \sigma(\alpha(\theta - H))$, verified formally, ensuring the gate is centered exactly at $\theta$ and monotonically decreasing in $H$.
...
\paragraph{H scorer---Semantic Harm Proximity (\emph{har\=arat al-kalima}).} The H scorer maintains 169 curated anchors spanning violence, weapons, self-harm, cybercrime, hate speech, and chemical/biological categories. Each anchor is a short phrase representing a harm concept. Input text is embedded via bge-m3 and compared against all anchors via cosine similarity. \emph{Top-$K$ aggregation} ($K=3$) averages the three highest-similarity scores, preventing any single anchor from dominating the result. This mitigates the \emph{anchor contamination} effect, in which one high-similarity anchor inflates the score despite low agreement across the anchor set. The H scorer includes 35 dialect-specific hyperbole anchors scored at $H \leq 0.10$, providing semantic ``counter-gravity'' for figurative Arabic expressions (Section~\ref{sec:arabic}).
...
AATIF includes 437 deterministic test cases (320 test functions, with parametrized expansions across multiple input scenarios---each parametrization is a real, independently executed test with its own pass/fail result) organized across the following categories: dialect detection (22~tests), S~equation mathematical properties (boundedness, monotonicity, sigmoid symmetry), decision logic across all four mode profiles, hysteresis stability, Law~$\Omega$ CBRN gate (12~tests), Law~$\Xi$ override lock, intent scorer (30~tests), adversarial cases (15~tests), gated comparison (18~tests), pipeline integration (3~tests), domain parameterization (35~tests), and held-out validation (2~tests). All tests are deterministic---they require no external model at test time and produce identical results across runs. The test suite runs via GitHub Actions for continuous integration. 437 tests pass.
...
AATIF provides an explicit, interpretable mathematical governance equation for LLM safety: $S = \sigma(w_1 \cdot I + w_2 \cdot E) \cdot [1 - \sigma(\alpha(H - \theta))]$. The multiplicative gate structure provides non-compensability by construction---high harm drives the safety score toward zero regardless of benign intent or calm emotion---with an external hard override ($H > 0.7$) supplying the absolute refusal floor. The framework requires zero training data and extends through anchor curation alone.

Arabic-first design is validated empirically: Arabic outperforms English on identical harmful prompts (74.7\% vs 69.3\% on MultiJail), and 35 dialect-hyperbole anchors eliminate false positives on figurative Arabic expressions. The system achieves 74.3\% safety-category detection on HarmBench with 100\% on chemical/biological threats, with in-sample calibration $F_1 = 0.984$ (54 cases) and held-out $F_1 = 0.9615$ (56 unseen cases) after anchor refinement, the latter still pending a fully blind validation round. A test suite of 437 deterministic test cases (320 test functions expanded via parametrization) ensures mathematical correctness.
...
\paragraph{Code and Data.} The AATIF computational engine, test suite (437 tests), benchmark runners, and evaluation harness are available at \url{https://github.com/Abdelmgied-Khenkar/AATIF}. The companion preprint is available at \url{https://doi.org/10.5281/zenodo.20673292}.
