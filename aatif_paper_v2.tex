...
Arabic-first design is validated empirically: Arabic and English achieve near-parity detection on identical harmful prompts (88.0\% and 90.7\% on MultiJail), with the multilingual bge-m3 backend closing the language gap observed in earlier scoring methods, and 35 dialect-hyperbole anchors (threshold $\leq 0.05$) eliminate false positives on figurative Arabic expressions. Run end-to-end through the full governance pipeline, the system blocks 83.8\% of HarmBench safety-category behaviors (and declines to execute 90.5\% once clarification requests are included), with 100\% block on chemical/biological, with in-sample calibration $F_1 = 0.984$ (54 cases) and held-out $F_1 = 1.0$ (56 cases) after iterative anchor refinement---noting that these cases informed the refinement, so a fully blind validation on an independent set is still required. A test suite of 932 deterministic tests ensures mathematical correctness across all modules.
